...
class AddController extends AbstractController
{
    ...
    public function action()
    {
        ...
        if ($this->routeExist($route)) {
            $error[] = "Route '$route' already exist (proxies which not created by database can't be overwrite).'";
        }
        $_SESSION['error'] = $error;
        if (!empty($error)) {
            return $this->getEngine()->render('main/redirect.php');
        }
        $this->getRoutesDao()->addRoute($route, $api, null, null, $responseContent);
        return $this->getEngine()->render('main/redirect.php');
    }
